...
// void init_acc() // initializes the accelerometer
// void read_acc() // loads global.correctedVectorGs with acc values in fixedpointnum g's
...
void read_acc() {
    ...
    ACC_ORIENTATION(global.correctedVectorGs, (((data[1]<<8) | data[0])>>2) * 64L,
...
void read_acc() {
    ...
    // convert readings to fixedpointnum (in g's)
    //usefull info is on the 14 bits  [2-15] bits  /4 => [0-13] bits  /4 => 12 bit resolution
    ACC_ORIENTATION(global.correctedVectorGs, (((data[0]<<8) | data[1])>>2) * 64L,
