/* 
Copyright 2013 Brad Quick, Jon Lochner
...
// char read_compass()  // loads global.compassVector[] with the compass vector as fixedpointnum.  
...
void compass_filter_raw_values(int *rawvalues) {
   for (int x=0;x<3;++x) {
...
void compass_read_raw_values(int *compassRawValues) {
...
char read_compass() {
    ...
    if (lib_timers_gettimermicroseconds(compassTimer)>=70000L) {
        ...
        compass_read_raw_values(compassRawValues);
...
void calibrate_compass() {
    ...
    compass_read_raw_values(rawvalues);
    ...
    while (lib_timers_gettimermicroseconds(testtimer)<30000000L) {
        ...
        while (lib_timers_gettimermicroseconds(compassTimer)<70000L) {}
        compass_read_raw_values(rawvalues);
        compass_filter_raw_values(rawvalues);
