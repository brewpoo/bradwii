...
// void init_baro()   // initializes the barometer and doesn't return until a good reading has been set
// char read_baro()   // sets global.baroRawAltitude in fixedpointnum meters  
...
void init_baro() {
    ...
    while (read_baro()==0) {} // make sure we start off with a good reading
}
...
char read_baro() {
    ...
    } else { //  we are waiting for pressure, wait 26 milliseconds
...
char read_baro() {
   ...
   if (lib_timers_gettimermicroseconds(ms561101ba_ctx.barotimer) < 10000) return 0;
